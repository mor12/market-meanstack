const CompraFree = require('../models/compra-free.model');
...
class CompraFreeController {
    post(req, res) {
        CompraFree.create([req.body], (err, r) => {
            if (err ) {
    ...
    get(req, res) {
        CompraFree.find().populate('employee').populate('user').exec((err, r) => {
            if (err) {
                return res.status(400).send({
                    message: errorHandler.getErrorMessage(err)
                });
    ...
    getComprasAttService(req, res) {
        ...
        CompraFree.find(filterOption).populate('employee').exec((err, r) => {
            if (err) {
                return res.status(400).send({
                    message: errorHandler.getErrorMessage(err)
                });
...
module.exports = new CompraFreeController;